...
- **Depends on** existing surfaces unchanged: your ad-platform data source (Google + Meta Ads, via MCP or API), GA4 (web analytics) tools, your CRM's MCP/API, the resolved CM artifact workspace (project-local or shared Drive), your browser-automation tool; your ad-platform daily-analysis methodology + account-economics as the methodology spine. If that methodology shifts, `cm-analyze`'s evidence base shifts under the whole chain.
