---
name: cm-review
description: "Compound Marketing — the REVIEW stage (Stage 4). Adversarial multi-lens review of an insights doc + marketing plan before execution. Dispatches 4 parallel lens agents (evidence, measurement, ownership, brand/client) and synthesizes findings. Use when the user says '/cm-review', 'review the marketing plan', 'adversarial review', 'stress-test the plan', 'check the plan before we execute', or after /cm-plan produces a plan doc."
---
...
This is the **review stage** — adversarially probe the plan before any execution resource is committed. Four independent lenses each review the same artifact for a different failure mode. Findings fold back into the plan or become Open questions for the operator. Nothing proceeds to Stage 5 without an approval gate here.
...
1. **The plan doc** — the `<Client> — Plan — <Channel> — <date>` doc in the flat `Compound Marketing` folder of your marketing docs store (Stage 3 output; search your marketing docs store — Google Drive, etc.)
2. **The insights doc** — the `<Client> — Analysis — <Channel> — <date>` doc in the same folder (Stage 2 output) — the plan flows from it; reviewers need both.
...
3. **`cm-lens-ownership`** — Is every action's owner accurate? Does the assigned owner (In-house / Copilot / Vendor / Auto) actually have the authority and access to execute it?
...
1. **Triage by severity:**
   ...
   - **P2** — notable gap or risk; surface as Open question for the operator
