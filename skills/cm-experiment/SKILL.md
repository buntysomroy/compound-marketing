...
6. **Write the Experiment artifact** in the resolved workspace. Project-local: `CM Artifacts/<project-slug>-<run-id>-<YYYY-MM-DD>-experiment.md`; shared Drive: `Experiment — <Channel / Topic> — <Client Display Name> — <YYYY-MM-DD>.docx`. Use the SOP template (Hypothesis → Mechanic → Baseline → Segments → Success Metrics + Guardrails → Decision Rule → Results → Implementation Notes). Render the design table inline for approval BEFORE anything goes live.
