...
> **Where this sits.**
> `/cm-audit` (Stage 1) → `/cm-analyze` (Stage 2) → **`/cm-plan` (this)** → `/cm-review` (Stage 4) → `/cm-agent-plan` (Stage 5a) → `/cm-execute` (Stage 5b).
>
...
7. **Hand off execution** — the artifact is a PLAN. Route execution to tactical `marketing-skills` (ads, cro, copywriting…) and to your message-drafting skill for any client/vendor comms. **Any action that changes state on a live client platform routes through `/cm-agent-plan` → `/cm-execute`'s Manifest-Gate model — Action Cards → Manifest Gate (`/cm-agent-plan`) → baseline → gate → act → read-back → receipt (`/cm-execute`) — never ad hoc browser automation or direct tool calls, even post-approval.** This is not optional once the plan is approved — approval authorizes the WORK, not a shortcut around the safety model that exists for exactly this failure class (a campaign build once ran ad hoc browser automation on a live client platform with no Manifest, no Action Cards, and no Effect Probe). Do not auto-send anything client-facing.
