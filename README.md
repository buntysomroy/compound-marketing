...
- **This plugin is now the primary CM surface (updated 2026-09-12).** RPD's own local `.claude/skills/cm-*` copies were deleted 2026-09-05 (#3770) — `compound-marketing@0.8.0`+ is the only CM surface, including inside RPD itself. Edit skills/reference docs directly in this repo; there is no separate upstream to re-publish from for the `cm-*` pipeline skills. (The publish SOP referenced below predates that migration and is kept for historical context / for any future skill that does start life RPD-local.)
